...
const FileExplorer: React.FC<FileExplorerProps> = ({
  ...
  const visibleItems = useMemo(() => {
    return searchQuery ? filterTreeData(fileTreeData, searchQuery) : fileTreeData
  ...
  const renderTreeItems = (
    ...
    return items.map((item) => {
      ...
      const folderState = isFolder ? getFolderSelectionState(item.value) : 'none'
  ...
  return (
    <div style={{ flexGrow: 1, overflow: 'auto', marginBottom: '10px' }}>
      {isLoading ? (
        ...
        <vscode-tree onvsc-tree-select={handleTreeSelect} expand-mode="singleClick" indent-guides>
